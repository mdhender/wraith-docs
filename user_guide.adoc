...
unskilled workers to load the cargo,
and plenty of storage to bring back anything we find.

TIP: The ship won't need `hyper-drive` engines since it's an in-system ship.

.In-System Survey Ship
|===
|Area|Unit|Mass per Unit|Volume per Unit|Quantity|Total Mass|Total Volume|Number of Structure Units Required
...
NOTE: The hull contains more `structural` units than it needs.
Those units (about 52,690) represent about 5,269 EMUs of unused cargo capacity.
